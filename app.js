...
const flightDetails = {
  outbound: {
    label: "查看 BR225 完整航班資料",
    image: "./assets/flight-br225.jpg",
    imageAlt: "BR225 去程訂位詳細資料",
    sections: [
      {
        title: "BR225｜台北 → 新加坡",
        rows: [
          "班機號碼：BR225",
          "航段：TPE 台北（桃園國際機場）→ SIN 新加坡（樟宜機場）",
          "出發：2026/08/15 07:30，桃園國際機場航站 2",
          "抵達：2026/08/15 11:55，樟宜機場航站 3",
          "機型：B777",
          "訂位艙等：經濟艙 N",
          "機位狀態：已確認"
        ]
      },
      {
        title: "搭機提醒",
        rows: [
          "04:30 兩組接送從新竹出發，先到桃園機場 T2。",
          "抵達機場後先確認護照、行李托運與登機門資訊。",
          "機上先補眠；抵達新加坡前請孩子把水壺喝完，入境後再補水。"
        ]
      }
    ],
    note: "截圖資訊來自訂位詳細資料；實際航廈、登機門與時間仍以航空公司與機場當日公告為準。"
  },
  inbound: {
    label: "查看 BR226 完整航班資料",
    image: "./assets/flight-br226.jpg",
    imageAlt: "BR226 回程訂位詳細資料",
    sections: [
      {
        title: "BR226｜新加坡 → 台北",
        rows: [
          "班機號碼：BR226",
          "航段：SIN 新加坡（樟宜機場）→ TPE 台北（桃園國際機場）",
          "出發：2026/08/19 13:10，樟宜機場航站 3",
          "抵達：2026/08/19 17:45，桃園國際機場航站 2",
          "機型：B777",
          "訂位艙等：經濟艙 N",
          "機位狀態：已確認"
        ]
      },
      {
        title: "返台銜接",
        rows: [
          "10:30 前進入報到與出境節奏，保留退稅、安檢與 8 人移動緩衝。",
          "抵達桃園 T2 後先領行李、確認兩組成員到齊。",
          "17:45 抵達後銜接兩組台灣機場接送回新竹。"
        ]
      }
    ],
    note: "截圖資訊來自訂位詳細資料；實際航廈、登機門與時間仍以航空公司與機場當日公告為準。"
...
const days = [
  {
    ...
    stops: [
      ...
      ["07:30", "台北 → 新加坡（BR225）", "直飛，抵達後以飯店休息與夜景為主。點開可看去程截圖、航廈、機型、艙等與機位狀態。", flightDetails.outbound],
      ...
      ["19:45", "珍寶海鮮 Riverside Point 晚餐", "第一晚安排辣椒螃蟹與海鮮桌菜，建議先訂位並點微辣版本；Merlion 到 Riverside Point 可短程叫車。"],
      ["21:15", "Spectra 水舞或回 Orchard", "若精神好再搭車回 MBS 看 Spectra / 夜景；若疲累，直接回 Holiday Inn Orchard 休息。"]
    ],
    tips: [
  ...
  },
  {
    ...
    stops: [
      ...
      ["16:45", "回 Holiday Inn Orchard 休息", "Science Centre 回 Orchard 約 25-35 分鐘。晚餐前先洗澡、補水、充電，避免晚上全員低電量。"],
      ["18:45", "松發肉骨茶 Clarke Quay 晚餐", "從 Orchard 到 New Bridge Road / Clarke Quay 很順，胡椒湯補體力，孩子可搭配油條、滷味與白飯。"],
      ["20:15", "Garden Rhapsody 燈光秀", "體力允許再搭車到 Supertree Grove 看燈光秀；若累，就留在 Clarke Quay 散步後回 Orchard。"]
    ],
    tips: [
      ["避暑", "8 月濕熱，上午花園冷房、下午科學館、晚間市區晚餐，是比硬走戶外更穩的親子節奏。"],
      ["科學館提醒", "Science Centre 通常週一休館，這天是星期日，安排很剛好。"],
      ["雨備", "午後雷陣雨時，Gardens 展館、MBS 商場、Science Centre 和 Orchard 商場都能銜接。"],
  ...
  },
  {
    ...
    tips: [
      ["行李策略", "最穩方案是 Holiday Inn 寄放行李，動物園後回 Orchard 取行李；若預算允許，包車轉場可省回頭路。"],
  ...
  },
  {
    ...
    stops: [
      ...
      ["09:15", "抵達 Resorts World Sentosa", "先拍地球標誌、確認 Express Pass 與當日開園時間。住在島上可比市區客更從容抵達。"],
      ...
      ["19:30", "聖淘沙散步與晚餐", "RWS、Beach Station 或 Oasia 附近晚餐都可。孩子若累，直接回飯店泡澡休息。"]
    ],
    tips: [
      ["優先順序", "USS 是今天主菜，Oceanarium 是漂亮加分項，不要為了全收集犧牲孩子心情。"],
      ["住宿優勢", "Oasia 在島上，午后若有人累了，可分組回飯店休息，其他人留在 RWS 繼續玩。"],
  ...
  },
  {
    ...
    stops: [
      ...
      ["08:15", "前往樟宜機場", "Oasia / 聖淘沙到機場抓 35-55 分鐘，尖峰或雨天再加緩衝；8 人加行李建議預約 van 或分 2 台車。"],
      ...
      ["13:10", "新加坡 → 台北（BR226）", "早上直攻星耀樟宜，保留報到緩衝。點開可看回程截圖、航廈、機型、艙等與機位狀態。", flightDetails.inbound],
    ...
    tips: [
      ...
      ["退稅", "若有購物退稅，請把單據與商品集中給一位成人管理；需查驗的商品不要太早托運。"],
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "看完水舞後可沿灣區步道慢走，或直接叫車回 Orchard 飯店，第一天不需要硬撐太晚。",
...
const stopMapQueries = {
  ...
  "前往魚尾獅公園": "Merlion Park Singapore",
  ...
  "回 Holiday Inn Orchard 休息": "Holiday Inn Singapore Orchard City Centre",
  ...
  "Holiday Inn 退房寄放行李": "Holiday Inn Singapore Orchard City Centre",
  "出發前往 Singapore Zoo": "Singapore Zoo",
  "新加坡動物園主園區": "Singapore Zoo",
  "園內午餐與表演": "Singapore Zoo Ah Meng Restaurant",
  "回 Holiday Inn 取行李": "Holiday Inn Singapore Orchard City Centre",
  "移動到 Oasia Resort Sentosa": "Oasia Resort Sentosa Singapore",
  "Oasia Resort Sentosa 入住": "Oasia Resort Sentosa Singapore",
  "Oasia 泳池與 Sentosa 慢散步": "Oasia Resort Sentosa Singapore",
  "聖淘沙晚餐": "Resorts World Sentosa Singapore",
  "Oasia 早餐與出門": "Oasia Resort Sentosa Singapore",
  ...
  "Oasia 早餐與退房": "Oasia Resort Sentosa Singapore",
...
const thumbImages = {
  ...
  orchard: "https://images.unsplash.com/photo-1519567241046-7f570eee3ce6?auto=format&fit=crop&w=360&q=78",
  holidayInn: "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=360&q=78",
  sentosa: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=360&q=78",
  oasia: "https://images.unsplash.com/photo-1571896349842-33c89424de2d?auto=format&fit=crop&w=360&q=78",
...
const stopThumbImages = {
  ...
  "回 Holiday Inn Orchard 休息": thumbImages.holidayInn,
  "松發肉骨茶 Clarke Quay 晚餐": thumbImages.bakKutTeh,
  "Garden Rhapsody 燈光秀": thumbImages.gardens,
  "Holiday Inn 退房寄放行李": thumbImages.holidayInn,
  "出發前往 Singapore Zoo": thumbImages.zoo,
  "新加坡動物園主園區": thumbImages.zoo,
  "園內午餐與表演": thumbImages.zoo,
  "回 Holiday Inn 取行李": thumbImages.holidayInn,
  "移動到 Oasia Resort Sentosa": thumbImages.oasia,
  "Oasia Resort Sentosa 入住": thumbImages.oasia,
  "Oasia 泳池與 Sentosa 慢散步": thumbImages.sentosa,
  "聖淘沙晚餐": thumbImages.rws,
  "Oasia 早餐與出門": thumbImages.oasia,
